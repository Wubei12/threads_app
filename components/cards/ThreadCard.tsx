...
interface Props {
    ...
    content: string, author: {
        ...
        image: string;
        id: string;
    }, community: { id: string; name: string; image: string; } | null, createdAt: string, comments: {
        author: {
            image: string;
        }
    }[]
            ...
            <div className="flex items-center justify-between">
                <div className="flex flex-row flex-1 w-full gap-4">
                    ...
                    <div className='flex flex-col w-full'>
                        <Link href={`/profile/${author.id}`} className='w-fit'> <h4 className="cursor-pointer text-base-semibold text-light-1"> {author.name} </h4> </Link>
